...
from engine.world_renderer import WorldRenderer
...
def main():
    ...
    while running:
        ...
        # Quick Win: Reduce mesh update frequency
        if frame_count % 30 == 0:  # Update every 30 frames instead of adaptively
        ...
        visible_chunks = world_manager.get_visible_chunks(camera, projection)
        chunks_rendered = 0

        # Process completed chunks from background threads (includes mesh upload)
        world_manager.process_completed_chunks()

        # Render all visible chunks in a single batched draw call
        world_renderer.render_chunks(visible_chunks)
        chunks_rendered = len(visible_chunks)
        ...
        if frame_count % 20 == 0:  # FIX: Update more frequently for better feedback
            ...
            pygame.display.set_caption(
                ...
                f"Gen Queue: {world_manager.chunk_queue.qsize() + world_manager.priority_queue.qsize()} | "
                f"Complete Queue: {completed_queue} | "
                f"Pos: ({camera.position[0]:.0f}, {camera.position[1]:.0f}, {camera.position[2]:.0f})"
            )
    
    # Cleanup
    print("\nShutting down...")
    world_renderer.cleanup()
